...
## 진행 현황 (2026-09-13 기준)
- ✅ **Phase A 완료·배포** — 글/댓글 수정, 공지 고정, (조회수는 사용자 요청으로 제거).
  SQL `011_board.sql` 실행 완료. 자체검증 통과.
- ✅ **Phase B 완료·배포** — 카테고리(잡담/질문/책추천/나눔/공지, 공지=관리자만),
  좋아요, 검색, 더보기. SQL `012_board_phase_b.sql` 실행 완료. 자체검증 통과.
  - 카테고리 상수: `src/lib/board.ts`. 기존 글은 전부 기본값 '잡담'.
  - board 읽기/쓰기 전부 service role 경유 + RLS 활성. profiles FK 없어 임베드 조인 대신 별도 조회.
- ⏸️ **Phase C 보류** — 이미지 첨부·경량 서식. 사용자가 여기서 마무리 결정.
  재개하려면 아래 "Phase C" 절대로 진행 + **Supabase Storage `board` 버킷(Public) 생성 필요**.
